...
async function check() {
  ...
  const users = await mongoose.connection.db.collection('users').find({ role: 'sales_manager' }).toArray();
  ...
  const txs = await mongoose.connection.db.collection('balancetransactions').find({ 
  ...
  // Check commissions
  const commissions = await mongoose.connection.db.collection('salescommissions').find({}).toArray();
  console.log('\n\n=== Sales Commissions ===');

...
async function fix() {
  ...
  const result1 = await db.collection('salescommissions').updateMany(
    { status: 'pending' },
    { $set: { status: 'PENDING' } }
  );
  console.log('Fixed pending:', result1.modifiedCount);
  
  const result2 = await db.collection('salescommissions').updateMany(
    { status: 'approved' },
    { $set: { status: 'APPROVED' } }
  );
  console.log('Fixed approved:', result2.modifiedCount);
  
  const result3 = await db.collection('salescommissions').updateMany(
    { status: 'paid' },
    { $set: { status: 'PAID' } }
  );
  console.log('Fixed paid:', result3.modifiedCount);
  
  const result4 = await db.collection('salescommissions').updateMany(
    { status: 'cancelled' },
  ...
  // Verify
  const commissions = await db.collection('salescommissions').find({}).toArray();
  console.log('\nAfter fix:');

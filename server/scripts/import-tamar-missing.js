...
const USER_ID = '68e9e11bf100794100883461';
const ARTIST_SLUG = 'tamarbasilia';
const ARTIST_BRAND = "Tamar Basilia's Art";
const MAX_SCAN = 700;
...
function belongsToArtist(message) {
...
async function main() {
  console.log(`Fetching ${ARTIST_SLUG} posts from Facebook page...`);
  ...
  while (scanned < MAX_SCAN) {
    ...
    for (const post of data.data) {
      scanned++;
      if (post.message && belongsToArtist(post.message) && !seenPostIds.has(post.id)) {
        seenPostIds.add(post.id);
        allPosts.push(post);
      }
    }

    console.log(`Scanned ${scanned}, found ${allPosts.length} matching posts`);
  ...
  console.log(`Total matching posts: ${allPosts.length}`);
  ...
  const existingProducts = await productsCol
    .find({ user: userObjectId }, { projection: { _id: 1, name: 1, price: 1 } })
    .toArray();
  ...
  for (let index = 0; index < allPosts.length; index++) {
    ...
    const existingById = await productsCol.findOne(
      ...
      { projection: { _id: 1, user: 1, name: 1 } },
    );
    if (existingById) {
      conflicts++;
    ...
    process.stdout.write(`[${index + 1}/${allPosts.length}] ${parsed.name} ... `);
    ...
    for (let imageIndex = 0; imageIndex < originalImages.length; imageIndex++) {
      try {
        ...
        const key = `products/${ARTIST_SLUG}-${originalId}-${imageIndex}.jpg`;

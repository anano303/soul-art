...
async function fixVariantStock() {
  ...
  try {
    ...
    const problematicProducts = await productsCollection
      .find({
        countInStock: 0,
    ...
    console.log(
      `Found ${problematicProducts.length} products with mismatched stock`,
    );
    ...
    for (const product of problematicProducts) {
      ...
      if (hasNoAttributes && product.countInStock === 0) {
        ...
        if (totalVariantStock > 0) {
          ...
          console.log(
            `  variant stocks: ${product.variants.map((v) => v.stock).join(', ')}`,
          );

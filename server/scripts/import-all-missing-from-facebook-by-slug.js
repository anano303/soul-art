...
function postBelongsToSlug(message, slug) {
  return extractAuthorSlug(message) === normalizeSlug(slug);
}
...
async function main() {
  ...
  console.log('Loading seller slugs from database...');
  ...
  console.log(`Loaded ${sellers.length} sellers with slugs`);
  console.log('Scanning Facebook page posts once...');
  ...
  for (const seller of sellers) {
    const slug = normalizeSlug(seller.artistSlug);
    if (!slug) continue;

    try {

    const matches = allPosts
      .filter((post) => post.message && postBelongsToSlug(post.message, slug))
    ...
    for (let index = 0; index < matches.length; index++) {
      try {
        ...
        process.stdout.write(`[${index + 1}/${matches.length}] ${slug}: ${parsed.name} ... `);
        ...
        for (let imageIndex = 0; imageIndex < originalImages.length; imageIndex++) {
          try {
            ...
            const key = `products/${slug}-${originalId}-${imageIndex}.jpg`;
    ...
    console.log('Summary:', summary);
      ...
      console.log(`Summary: ${slug} failed with error: ${error.message}`);
      continue;
    }
  }

  console.log('\n=== FINAL SUMMARY ===');
  summaries.forEach((summary) => console.log(JSON.stringify(summary)));

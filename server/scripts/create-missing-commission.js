...
async function createMissingCommission() {
  ...
  if (existingCommission) {
    console.log('Commission already exists:', existingCommission._id.toString());
    await mongoose.disconnect();
    return;
  }
  
  // Find sales manager
  const salesManager = await db.collection('users').findOne({ salesRefCode: order.salesRefCode });

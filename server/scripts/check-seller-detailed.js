...
async function checkSellerDetailed(sellerName) {
  try {
    ...
    console.log('\n📋 Campaign Settings:');
    console.log(`   campaignDiscountChoice: ${seller.campaignDiscountChoice || 'NOT SET'}`);
    console.log(`   defaultReferralDiscount: ${seller.defaultReferralDiscount || 0}%`);
    ...
    if (seller.campaignConsent) {
      ...
      console.log(`   discountPercent: ${seller.campaignConsent.discountPercent}%`);
      console.log(`   consentDate: ${seller.campaignConsent.consentDate}`);
    }

    // Get all products for this seller, sorted by creation date
    const allProducts = await productsCollection.find({ user: seller._id })
      .sort({ createdAt: 1 })
      .toArray();
    ...
    allProducts.forEach((p, i) => {
      ...
      const createdAt = p.createdAt ? new Date(p.createdAt).toISOString().split('T')[0] : 'unknown';
      
      console.log(`${(i + 1).toString().padStart(2)}. [${createdAt}] ${promo > 0 ? '✅' : '❌'} ${promo}% | useDefault:${useDefault} | ${name.substring(0, 40)}`);
    });

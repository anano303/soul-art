...
async function setManagedBrands() {
  ...
  console.log('Found user:', user.name, '| ID:', user._id, '| Role:', user.role);
  ...
  console.log('\n=== ALL UNIQUE BRANDS ===');
  allBrands.filter(Boolean).sort().forEach(b => console.log(' -', b));
  ...
  for (const brand of matchingBrands) {
    ...
    for (const ownerId of ownerIds) {
      const owner = await db.collection('users').findOne({ _id: require('mongodb').ObjectId.createFromHexString(ownerId) });
  ...
  if (matchingBrands.length > 0) {
    const result = await db.collection('users').updateOne(
      { email },
      { $set: { managedBrands: matchingBrands } }
    );
    console.log('\n=== UPDATE RESULT ===');

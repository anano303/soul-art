...
async function fixPortfolio() {
  ...
  const correctImage =
    'https://res.cloudinary.com/dpgmovzur/image/upload/v1749826590/products/ggjhpdw7vy8k2rq9m2qy.jpg';
  ...
  const result = await db.collection('portfolioposts').updateOne(
    { _id: new ObjectId('694ff71d2cfb62c49454d285') },
    {
      $set: {
        title: 'უსახელო',
        description:
          'ნახატი გადმოსცემს ბავშვურ გულწრფელობას, სითბოსა და სიჩუმეში ჩაკარგულ ოცნებას, თითქოს პატარა ზღაპრის ერთი კადრია.',
        updatedAt: new Date(),
      },
    },
  );
  ...
  const post = await db
    ...
    .findOne({ _id: new ObjectId('694ff71d2cfb62c49454d285') });
  console.log('\nCurrent post:');

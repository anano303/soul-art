#!/usr/bin/env node
/**
 * Import FB products for abramishvilidaviti.98@gmail.com (KKL beads / kkl-beads)
 * Creates products + portfolio posts + English translations.
 *
 * Usage:
 *   node scripts/import-fb-kkl-beads.js          # dry run
 *   node scripts/import-fb-kkl-beads.js --apply   # real run
 */
...
const TARGET = {
  email: 'abramishvilidaviti.98@gmail.com',
  slug: 'kkl-beads',
  displayName: 'KKL beads',
};
...
function detectCategory(parsed) {
  ...
  // KKL beads override: if materials contain მძივ/ძუა/beads, it's handmade (not painting)
  const isBeadsMaterial = /მძივ|ძუა|beads/.test(materialsText);
  ...
  if (!isBeadsMaterial && (paintingMaterials.test(materialsText) || /ნახატ|painting|paint|ფერწერ/.test(allText))) {
    const signals = {
  ...
  // KKL beads — default to სამკაულები (Jewelry) since it's a beads store
  const sc = SUBCATS['სამკაულები'];
  return { mainCategory: new ObjectId(sc.cat), mainCategoryEn: sc.catEn, subCategory: new ObjectId(sc.id), subCategoryEn: sc.en, category: 'ხელნაკეთი ნივთები', detectedAs: 'სამკაულები (default for beads store)' };
}

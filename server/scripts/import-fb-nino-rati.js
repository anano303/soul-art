#!/usr/bin/env node
/**
 * Import FB products for Nino Gudadze (ninogudadzeart) and Rati (ratigogshelidze)
 * 
 * Usage:
 *   node scripts/import-fb-nino-rati.js          # dry run
 *   node scripts/import-fb-nino-rati.js --apply   # real run
 */
...
const TARGETS = [
  { email: 'ninigudadze@gmail.com',       slug: 'ninogudadzeart',   displayName: 'ნინო გუდაძე' },
  { email: 'ratigogshelidze@gmail.com',    slug: 'ratigogshelidze',  displayName: 'რატი205' },
];
...
async function main() {
  console.log(`\n=== Import FB Products: Nino Gudadze + Rati ===`);
  ...
  for (const t of TARGETS) {
    const user = await usersCol.findOne({ email: t.email });
    if (!user) { console.log(`❌ ${t.email}: not found`); continue; }
    sellerMap[t.slug] = { userId: user._id, storeName: t.displayName };
    console.log(`✔ ${t.displayName} (${t.email}) → @${t.slug}`);
  }

  // Fetch FB posts
  const allPosts = await fetchAllPagePosts();

  for (const slug of Object.keys(sellerMap)) {
    const seller = sellerMap[slug];
    const matches = allPosts
      .filter(p => p.message && extractAuthorSlug(p.message) === slug)
      .sort((a, b) => new Date(a.created_time) - new Date(b.created_time));

    console.log(`\n── @${slug}: ${matches.length} FB posts ──`);
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      try {
        ...
        if (!originalId) { console.log(`  [${i+1}] "${parsed.name}" - NO ID, skip`); skipped++; continue; }

        const exists = await productsCol.findOne({ _id: new ObjectId(originalId) });
        if (exists) { console.log(`  [${i+1}] "${parsed.name}" - ALREADY EXISTS (owner: ${exists.user}), skip`); skipped++; continue; }

        const catInfo = detectCategory(parsed);

        if (!APPLY) {
          console.log(`  🟡 [${i+1}] "${parsed.name}" → ${catInfo.detectedAs} | ₾${parsed.price} | ${parsed.materials.join(', ') || '-'}`);
          imported++;
          continue;
        }
        ...
        for (let imgIdx = 0; imgIdx < originalImages.length; imgIdx++) {
          try {
            ...
            s3Images.push(s3Url);
          } catch { s3Images.push(originalImages[imgIdx]); }
        }

...
const ALL_CLOUD_NAMES = ['dsufx8uzd', 'dwfqjtdu2', 'dmvh7vwpu', 'dbz1wfzzj', 'dbzlwfzzj'];
...
function isCloudinaryUrl(url: string): boolean {
  ...
  return ALL_CLOUD_NAMES.some(name => url.includes(name)) || url.includes('res.cloudinary.com');
}
...
function normalizeCloudinaryUrl(url: string): string {
  ...
  for (const oldName of OLD_CLOUD_NAMES) {
    normalized = normalized.replace(new RegExp(oldName, 'g'), ACTIVE_CLOUD_NAME);
  }
  return normalized;
}
...
function extractCloudinaryPath(url: string): { folder: string; filename: string; extension: string; resourceType: string } {
  try {
    const urlObj = new URL(url);
    const segments = urlObj.pathname.split('/').filter(Boolean);

    // Find resource type (image/video/raw)
    const resourceTypeIndex = segments.findIndex(s => ['image', 'video', 'raw'].includes(s));
    const resourceType = resourceTypeIndex >= 0 ? segments[resourceTypeIndex] : 'image';

    // Get path after 'upload' and optional version
    const uploadIndex = segments.indexOf('upload');
    let pathSegments = uploadIndex >= 0 ? segments.slice(uploadIndex + 1) : segments;
    ...
    if (pathSegments.length === 0) {
      ...
      return { folder: 'migrated', filename: hash, extension: '.jpg', resourceType };
    }

    const lastSegment = pathSegments[pathSegments.length - 1];
    const extMatch = lastSegment.match(/\.([^.]+)$/);
    const extension = extMatch ? `.${extMatch[1].toLowerCase()}` : '.jpg';
    const filenameWithoutExt = extMatch ? lastSegment.slice(0, -(extMatch[1].length + 1)) : lastSegment;
    ...
    const hash = crypto.createHash('md5').update(url).digest('hex');
    return { folder: 'migrated', filename: hash, extension: '.jpg', resourceType: 'image' };
  }
}
...
async function downloadImage(url: string): Promise<{ buffer: Buffer; contentType: string }> {
  return new Promise((resolve, reject) => {
    ...
    protocol.get(url, options, (response) => {
      ...
      response.on('end', () => {
        const ct = (Array.isArray(response.headers['content-type'])
...
async function getAllCloudinaryUrls(db: any): Promise<Map<string, Array<{ collection: string; docId: string; field: string; arrayIndex?: number }>>> {
  const urlMap = new Map<string, Array<{ collection: string; docId: string; field: string; arrayIndex?: number }>>();

  const addUrl = (url: string, collection: string, docId: string, field: string, arrayIndex?: number) => {
  ...
  if (!COLLECTION_FILTER || COLLECTION_FILTER === 'products') {
    ...
    for (const p of products) {
      if (p.images && Array.isArray(p.images)) {
        p.images.forEach((img: string, i: number) => addUrl(img, 'products', p._id.toString(), 'images', i));
      }
      if (p.brandLogo) addUrl(p.brandLogo, 'products', p._id.toString(), 'brandLogo');
      if (p.thumbnail) addUrl(p.thumbnail, 'products', p._id.toString(), 'thumbnail');
    }
  }
  ...
  if (!COLLECTION_FILTER || COLLECTION_FILTER === 'users') {
    ...
    for (const u of users) {
      if (u.profileImagePath) addUrl(u.profileImagePath, 'users', u._id.toString(), 'profileImagePath');
      if (u.storeLogoPath) addUrl(u.storeLogoPath, 'users', u._id.toString(), 'storeLogoPath');
      if (u.storeLogo) addUrl(u.storeLogo, 'users', u._id.toString(), 'storeLogo');
      if (u.artistCoverImage) addUrl(u.artistCoverImage, 'users', u._id.toString(), 'artistCoverImage');
      if (u.artistGallery && Array.isArray(u.artistGallery)) {
        u.artistGallery.forEach((img: string, i: number) => addUrl(img, 'users', u._id.toString(), 'artistGallery', i));
      }
    }
  }
  ...
  if (!COLLECTION_FILTER || COLLECTION_FILTER === 'banners') {
    ...
    for (const b of banners) {
      if (b.imageUrl) addUrl(b.imageUrl, 'banners', b._id.toString(), 'imageUrl');
    }
  }
  ...
  if (!COLLECTION_FILTER || COLLECTION_FILTER === 'forums') {
    ...
    for (const f of forums) {
      if (f.imagePath) addUrl(f.imagePath, 'forums', f._id.toString(), 'imagePath');
    }
  }
  ...
  if (!COLLECTION_FILTER || COLLECTION_FILTER === 'blogposts') {
    ...
    for (const b of blogs) {
      if (b.coverImage) addUrl(b.coverImage, 'blogposts', b._id.toString(), 'coverImage');
      if (b.images && Array.isArray(b.images)) {
        b.images.forEach((img: string, i: number) => addUrl(img, 'blogposts', b._id.toString(), 'images', i));
      }
    }
  }
    ...
    for (const p of portfolios) {
      if (p.images && Array.isArray(p.images)) {
        p.images.forEach((img: any, i: number) => {
          if (img && img.url) addUrl(img.url, 'portfolioposts', p._id.toString(), 'images', i);
        });
      }
    }
  }
...
async function updateDbUrls(
  db: any,
  urlMap: Map<string, Array<{ collection: string; docId: string; field: string; arrayIndex?: number }>>,
  oldUrl: string,
  newUrl: string,
): Promise<number> {
    ...
    try {
      if (ref.arrayIndex !== undefined) {
        ...
        if (ref.field === 'images' && ref.collection === 'portfolioposts') {
          ...
          await db.collection(ref.collection).updateOne(
      ...
      console.error(`  ❌ Failed to update ${ref.collection}.${ref.docId}.${ref.field}: ${error.message}`);
    }
  }
    ...
    urls.map(async (url) => {
      ...
      try {
        ...
        if (DRY_RUN) {
          ...
          console.log(`  [DRY-RUN] Would upload: ${s3Key} (${(buffer.length / 1024).toFixed(1)}KB)${wasNormalized}`);
          stats.migrated++;
          return;
        }
...
async function main() {
  ...
  if (previouslyMigrated > 0) {
    console.log(`📋 Loaded previous progress: ${previouslyMigrated} URLs already migrated\n`);
  }
  ...
  for (const [, refs] of urlMap) {
    for (const ref of refs) {
      collectionCounts[ref.collection] = (collectionCounts[ref.collection] || 0) + 1;
    }
  }
  ...
  if (oldCloudNameUrls.length > 0) {
    ...
    for (const cn of OLD_CLOUD_NAMES) {
      ...
      if (count > 0) console.log(`   ${cn} → ${ACTIVE_CLOUD_NAME}: ${count} URLs`);
    }
  }
  ...
  console.log(`⏳ Processing ${allUrls.length} URLs in batches of ${BATCH_SIZE}...\n`);
  ...
  for (let i = 0; i < allUrls.length; i += BATCH_SIZE) {
    ...
    process.stdout.write(`\r  📈 Progress: ${processed}/${allUrls.length} (${pct}%) | ✅ ${stats.migrated} | ⏭️ ${stats.skipped} | ❌ ${stats.failed} | 🔄 ${stats.normalizedUrls} | ⏱️ ${elapsed}s`);
  }
  ...
  console.log(`  🔄 Normalized:  ${stats.normalizedUrls} (ძველი cloud name → ${ACTIVE_CLOUD_NAME})`);

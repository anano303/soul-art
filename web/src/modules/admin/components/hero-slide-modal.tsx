...
export function HeroSlideModal({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "სლაიდი შეიქმნა",
        description: "Hero სლაიდი წარმატებით დაემატა",
      });
      onSuccess();
    },
    onError: () => {
      toast({
        title: "შეცდომა",
        description: "სლაიდის შექმნა ვერ მოხერხდა",
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "სლაიდი განახლდა",
        description: "Hero სლაიდი წარმატებით განახლდა",
      });
      onSuccess();
    },
    onError: () => {
      toast({
        title: "შეცდომა",
        description: "სლაიდის განახლება ვერ მოხერხდა",
        variant: "destructive",
      });
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isEditing && !selectedImage) {
      toast({
        title: "შეცდომა",
        description: "გთხოვთ აირჩიოთ სლაიდის სურათი",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2 className="modal-title">
            {isEditing ? "სლაიდის რედაქტირება" : "ახალი Hero სლაიდი"}
          </h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={handleInputChange}
              className="form-input"
              placeholder="https://example.com (არასავალდებულო)"
            />
          </div>

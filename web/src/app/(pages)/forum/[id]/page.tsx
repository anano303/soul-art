...
export async function generateMetadata({ 
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/v1'}/forums/${params.id}`, {
    ...
    const title = `${truncateText(textContent, 60)} - ${forum.user.name} | SoulArt.ge Forum`;
    ...
    return {
      ...
      openGraph: {
        title,
        description,
        url: `https://soulart.ge/forum/${params.id}`,
        siteName: 'SoulArt.ge',
        images: forum.image ? [
          {
            url: forum.image,
      ...
      alternates: {
        canonical: `https://soulart.ge/forum/${params.id}`,
      },
    ...
    return {
      ...
      description: 'Discover Georgian art and connect with artists on SoulArt.ge forum.',
    };
  }
}
...
export default async function ForumPostPage({ 
  ...
  return <SingleForumPost postId={params.id} />;
}